...
extern struct doca_dev *doca_devs[RTE_MAX_ETHPORTS];
extern bool doca_devs_owned[RTE_MAX_ETHPORTS];
extern bool doca_devs_bridge_mapped[RTE_MAX_ETHPORTS];
...
continue_dev_start:
	...
	if (doca_devs[dev->data->port_id] == NULL) {
		ret = rte_pmd_mlx5_doca_bridge_port_prepare(dev->data->port_id);
		if (ret == 0) {
			DRV_LOG(INFO, "port %u found pre-established DOCA-DPDK bridge association",
				dev->data->port_id);
		} else {
			pci_dev = RTE_DEV_TO_PCI(dev->device);
			if (pci_dev == NULL) {
				DRV_LOG(ERR, "port %u is not backed by a PCI device", dev->data->port_id);
				rte_errno = ENODEV;
				SAVE_RTE_ERRNO_AND_STOP(ret, dev);
				goto rxq_stop;
			}
			rte_pci_device_name(&pci_dev->addr, pci_addr, sizeof(pci_addr));
			doca_status = open_doca_device_with_pci(pci_addr, NULL,
						      &doca_devs[dev->data->port_id]);
			if (doca_status != DOCA_SUCCESS || doca_devs[dev->data->port_id] == NULL) {
				DRV_LOG(ERR, "port %u failed to open DOCA device for PCI %s: %s",
					dev->data->port_id, pci_addr, doca_error_get_name(doca_status));
				rte_errno = ENODEV;
				SAVE_RTE_ERRNO_AND_STOP(ret, dev);
				goto rxq_stop;
			}
			doca_devs_owned[dev->data->port_id] = true;
			doca_devs_bridge_mapped[dev->data->port_id] = false;
			DRV_LOG(WARNING,
				"port %u has no DOCA-DPDK bridge association; using direct DOCA device fallback",
				dev->data->port_id);
		}
	}
	
	ret = rxq_doca_start(dev->data->port_id, dev->data->nb_rx_queues,
			     doca_devs[dev->data->port_id],
			     doca_devs_bridge_mapped[dev->data->port_id]);
	if (ret == DOCA_ERROR_NOT_SUPPORTED) {
		DRV_LOG(WARNING,
			"port %u skipping DOCA flow setup because the DOCA-DPDK bridge is not available for this port",
			dev->data->port_id);
		ret = 0;
	}
	if (ret != DOCA_SUCCESS) {
		DRV_LOG(ERR, "port %u DOCA flow setup failed: %s",
			dev->data->port_id, doca_error_get_name(ret));
		rte_errno = EIO;
		SAVE_RTE_ERRNO_AND_STOP(ret, dev);
		goto rxq_stop;
	}
...
rxq_stop:
	rxq_doca_stop(dev->data->port_id);
	mlx5_rxq_stop(dev);
txq_stop:
	mlx5_txq_stop(dev);
	if (doca_devs[dev->data->port_id] != NULL && doca_devs_owned[dev->data->port_id]) {
		doca_status = doca_dev_close(doca_devs[dev->data->port_id]);
		if (doca_status != DOCA_SUCCESS)
			DRV_LOG(WARNING, "port %u failed to close DOCA device: %s",
				dev->data->port_id, doca_error_get_name(doca_status));
		doca_devs_owned[dev->data->port_id] = false;
	}
	doca_devs[dev->data->port_id] = NULL;
	doca_devs_bridge_mapped[dev->data->port_id] = false;
free_consec_tx_mem:
...
continue_dev_stop:
	...
	mlx5_txpp_stop(dev);
	if (doca_devs[dev->data->port_id] != NULL && doca_devs_owned[dev->data->port_id]) {
		doca_error_t doca_status = doca_dev_close(doca_devs[dev->data->port_id]);
		if (doca_status != DOCA_SUCCESS)
			DRV_LOG(WARNING, "port %u failed to close DOCA device: %s",
				dev->data->port_id, doca_error_get_name(doca_status));
		doca_devs_owned[dev->data->port_id] = false;
